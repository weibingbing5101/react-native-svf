...
import {Input, Button,LabelInput,RowButton,InfoShow} from '../../Form';
...
export default class CardApply extends Component {
  ...
  render() {
    ...
    return (
        ...
        <ScrollView style={[stylePos,{backgroundColor:'white'}]} ref="scroller" onScroll={ this._onScroll } scrollEventThrottle={10} keyboardDismissMode='on-drag'>
          <LabelInput {...this.props}
            ...
            tip={!this.state.fastNameIsValid}/> 
          <LabelInput 
            styles={styles}
            label="名" 
            leftSize = {74}
            ...
            tip={!this.state.lastNameIsValid} />
          <LabelInput 
            styles={styles}
            label="卡签名" 
            leftSize = {74}
            ...
            minLength={2} 
            maxLength={20} 
            defaultValue={this.state.info.signature} 
            keyboardType="name-phone-pad" 
            onChange={this.onChangeSignature} 
            tip={!this.state.signatureIsValid}/>
          <LabelInput {...this.props}
            label="账户币种"
            leftSize = {74}
            defaultValue="港元" 
