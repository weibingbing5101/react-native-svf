...
const topHeight = 75;
export default class WithholdProtocol extends Component {
  ...
  render() {
    ...
    let h5Url = "http://goopalcard.xiaojian.me/gc-ios/GopWithholdProtocolpdf.html?t="+Date.now();

    // let h5Url = 'http://baidu.com';7200
    h5Url =  {uri:h5Url};

    let webViewHeight = 3072;
    if (width>400) {
      webViewHeight = 3380;
    }
    return (
      <View style={{flex:1}}>
        <CustomBackTitle title="果仁宝账户代扣服务协议" backOnly={true} left={-width/2+30} {...this.props}/>
        <ScrollView>
          <WebView 
            source={ h5Url }
